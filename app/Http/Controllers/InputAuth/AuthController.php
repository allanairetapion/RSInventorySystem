    ...
    */
	...
	$is_users = IsUser::where('email', '=', $data['email']) -> first();
     ...
     //$url = URL::to('/inventory/verify/'.$confirmation_code); 
        Mail::send('inventory.verifyEmail', ['is_users' => $is_users], function ($message) use ($is_users)
...
public function confirm($confirmation_code)
    {
        ...
        return view('/inventory/verified',array("is_users" => $is_users));
    }
   ...
   public function postLogin(Request $request)
    {
		...
		$post_data = $request -> all();
				...
				$request['password'] = $post_data['password'];
    			...
    			else
    			{
    				...
    				if(Hash::check($request['password'], $is_users['password']))
    				{
								
						if($is_users['confirmed'] == 1 )
						{			
						...
						else
						{
							...
							return view("/inventory/notVerified", array("is_users" => $is_users));
						}
...
public function resendVerLink($confirmation_code)
{
	...
	return view("inventory.signuptypage", array("is_users" => $is_users));
